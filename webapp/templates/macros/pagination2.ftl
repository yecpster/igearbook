...
<#macro doPagination action id=-1>
	<#if (totalRecords > recordsPerPage)>
		...
		<#if (thisPage > 1)>
			...
			<a href="${contextPath}/${moduleName}/${action}<#if (start > 0)>/${start}</#if><#if (id > -1)>/${id}</#if>${extension}" class="prev">‹‹</a>
		</#if>
		...
		<#if (thisPage < totalPages)>
			...
			<a href="${contextPath}/${moduleName}/${action}<#if (start > 0)>/${start}</#if><#if (id > -1)>/${id}</#if>${extension}" class="next">››</a>
		</#if>
		<kbd>
			<input type="text" name="custompage" size="3"  id="pageToGo" onkeydown="if(event.keyCode==13){goToAnotherPage(${totalPages}, ${recordsPerPage}, '${contextPath}', '${moduleName}', '${action}', ${id}, '${extension}');}" />
			<input type="text" readonly="readonly" size="4" value="${I18n.getMessage("ForumIndex.goToGo")}&#9658;" onClick="goToAnotherPage(${totalPages}, ${recordsPerPage}, '${contextPath}', '${moduleName}', '${action}', ${id}, '${extension}');">
		</kbd>
		</div>
	</#if>
</#macro>
