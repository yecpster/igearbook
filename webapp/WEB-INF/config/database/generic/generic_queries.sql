...
GroupModel.selectCandidateGroups = SELECT group_id, group_name, parent_id, group_description FROM jforum_groups where group_type = 0 and group_id not in (#IN#) ORDER BY group_name
GroupModel.selectById = SELECT group_id, group_name, parent_id, group_description FROM jforum_groups WHERE group_id = ?
GroupModel.selectByName = SELECT group_id, group_name, parent_id, group_description FROM jforum_groups WHERE group_name = ?
GroupModel.selectParentGroups = SELECT g.group_id, g.group_name FROM jforum_groups g INNER JOIN jforum_groups_groups gg on (g.group_id = gg.group1_id and gg.relation = 0) WHERE gg.group2_id = ?
GroupModel.selectChildGroups = SELECT g.group_id, g.group_name, g.group_description FROM jforum_groups g INNER JOIN jforum_groups_groups gg on (g.group_id = gg.group2_id and gg.relation = 0) WHERE gg.group1_id = ?
...
GroupModel.addNew = INSERT INTO jforum_groups (group_name, group_description, parent_id, group_type) VALUES (?, ?, ?, ?)
GroupModel.selectUsersIds = SELECT user_id FROM jforum_user_groups WHERE group_id = ?
GroupModel.deleteChilds = DELETE FROM jforum_groups_groups WHERE relation = 0 and group1_id = ? 
GroupModel.appendChild = INSERT INTO jforum_groups_groups (group1_id, group2_id) VALUES (?, ?)
...
UserModel.login = SELECT user_id FROM jforum_users WHERE LOWER(username) = LOWER(?) AND user_password = ? and (apiUser = 0 or apiUserActive=1)
...
RecommendModel.addNew = INSERT INTO jforum_recommendation (recommend_type, image_url, topic_id, topic_title, topic_desc, create_user_id, last_update_user_id, create_time, last_update_time) \
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())
RecommendModel.update = UPDATE jforum_recommendation set recommend_type=?, image_url=?, topic_title=?, topic_desc=?, last_update_user_id=?, last_update_time=? \
    WHERE recommend_id = ?
...
ForumModel.lastPostInfo = SELECT post_time, p.topic_id, t.topic_title, t.topic_replies, post_id, u.user_id, username \
...
PermissionControl.selectForumRoles = SELECT DISTINCT r.role_id FROM jforum_roles r, jforum_role_values rv \
	...
	AND r.name in ('perm_forum', 'perm_anonymous_post', 'perm_new_post', 'perm_reply', 'perm_reply_without_moderation', 'perm_html_disabled', 'perm_attachments_enabled', 'perm_moderation_forums') \
	AND rv.role_value = ?
